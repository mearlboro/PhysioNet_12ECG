...
def get_peak_measures(data,fs, lead):
    ...
    '''
    peak_props = dict()
    #lead_str = ['I','II','III','aVR','aVL','aVF','V1','V2','V3','V4','V5','V6']
    data = data[:,lead]
    ...
    if(len(peaks)>int(0.5*len(data1)/fs)):
        ...
        peak_props['max_hrs'] = np.max(hrs)
        peak_props['min_hrs'] = np.min(hrs)
        ...
        t_prop_names = list(t_props[0].keys())
        ...
        p_props_mean = np.nanmean(p_props,axis=0)
        p_props_std = np.nanstd(p_props,axis=0)
        ...
        dist_props = get_dist_props(data1, q_begs, r_peaks, s_ends, t_begs, t_ends, p_begs, p_ends, peak_props['mean_hrs'], fs)
        ...
        peak_props['qtcorr1mean'] = peak_props['qt_dist_mean']/ np.sqrt(peak_props['mean_hrs'])
        peak_props['qtcorr2mean'] = peak_props['qt_dist_mean']/ (peak_props['mean_hrs']**(1./3))
        peak_props['qtcorr3mean'] = peak_props['qt_dist_mean'] + 0.154 * (1 - peak_props['mean_hrs'])
        peak_props['qtcorr4mean'] = peak_props['qt_dist_mean'] + 1.75 * ((60./peak_props['mean_hrs']) - 60)
...
def get_power_measures(data,fs, lead):
    ...
    pow_props['mean_power'] = np.mean(power_rel)
    pow_props['std_power'] = np.std(power_rel)
    pow_props['iqr_power'] = iqr(power_rel)
    pow_props['max_power'] = np.max(power_rel)
    pow_props['min_power'] = np.min(power_rel)
    
    ratios = power_rel/pow_props['mean_power']
